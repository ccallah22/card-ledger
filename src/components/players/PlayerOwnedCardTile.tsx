...
export type PlayerOwnedCardTileProps = {
  ...
  // card_media image, resolved by the caller the same way (via
  // useUserCardDisplayImages(ids, "back")), batched at the page level --
  // never fetched per-tile. null (still resolving, or genuinely no back
  // image) means this card is not flippable, a normal state, not an error.
  backUrl?: string | null;
...
export function PlayerOwnedCardTile({
  ...
  return (
    <div className="group relative h-full">
      {/* Binder Card Flip: the real navigating <a> is a separate, invisible,
          full-tile overlay rather than the element that visually contains
          the image/text below -- see CardTile.tsx's identical pattern and
          its own comment for why (a flip <button> can never validly be a
          descendant of an <a>). z-10 keeps it beneath the flip button once
          one exists. */}
      <Link
      ...
      <div
        className={
          "pointer-events-none block h-full rounded-xl border bg-white p-3 shadow-sm transition group-hover:-translate-y-0.5 group-hover:shadow-md " +
          (featured
            ? "border-[var(--brand-primary)] ring-2 ring-[var(--brand-primary)]/30"
            : "border-zinc-200")
        }
      >
        {/* No pointer-events override here: FlippableCardImage's own flip
            <button> (rendered only once backUrl exists) re-enables
            pointer-events on itself directly, regardless of this
            ancestor's pointer-events-none -- see CardTile.tsx's identical
            comment. With no back image, this box must stay click-through
            so the plain <img> lets clicks fall to the stretched Link. */}
        <div className="relative aspect-[2.5/3.5] w-full overflow-hidden rounded-lg border border-zinc-200 bg-gradient-to-br from-white via-zinc-50 to-zinc-100 flex items-center justify-center">
          {featured ? (
            <span className="pointer-events-none absolute left-1.5 top-1.5 z-10 rounded-full bg-[var(--brand-primary)] px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-white shadow">
              Top Card
            </span>
          ) : null}
          {imageUrl ? (
            // imageUrl is a signed Supabase Storage URL resolved by the
            // caller (see PlayerOwnedCardTileProps.imageUrl above); backUrl
            // (see above) comes only from that same resolver's "back" side,
            // never a community-reference fallback (this page has none).
            <FlippableCardImage
              frontUrl={imageUrl}
